...
function Login() {
    ...
    const { backendURL, setisprevious, setusername, setislogged, setrole, islogged, role } = useContext(usercontext)
    ...
    useEffect(() => {
        if (islogged) {
            navigate(role === "JOB_RECRUITER" ? "/recruiter-dashboard" : "/")
        }
    }, [islogged, role])
    ...
    const submit = (event) => {
        ...
        if (!islogin) {
            ...
            fetch(`${backendURL}/login`, {
                method: "post", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ email: email.trim(), password: password, role: selectedRole }), credentials: 'include'
            })
                .then(response => {
                    if (response.ok) {
                        setemail("")
                ...
                .then(data => {
                    if (data != null) {
                        ...
                        setrole(data.role)
                        navigate(data.role === "JOB_RECRUITER" ? "/recruiter-dashboard" : "/")
                    }
                })
                .catch(error => {
    ...
    const verifyprocess = () => {
        ...
        fetch(`${backendURL}/register`, {
            ...
            body: JSON.stringify({ "username": name.trim(), "email": email.trim(), "password": password, "role": selectedRole, "verifyotp": enteredOtp })
        ...
        <div className={Styles.container}>
            ...
            {!isemailverified ? <div className={Styles.logincontainer}>
                ...
                <div className={Styles.rolecontainer}>
                    <label>
                        <input type="radio" name="userrole" value="JOB_SEEKER" checked={selectedRole === "JOB_SEEKER"} onChange={(event) => setselectedRole(event.target.value)} />
                        Job Seeker
                    </label>
                    <label>
                        <input type="radio" name="userrole" value="JOB_RECRUITER" checked={selectedRole === "JOB_RECRUITER"} onChange={(event) => setselectedRole(event.target.value)} />
                        Job Recruiter
                    </label>
                </div>
                {
