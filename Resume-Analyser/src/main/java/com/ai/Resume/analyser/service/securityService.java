    ...
    public ResponseEntity<?> login(@Valid userLogin req) {
        ...
        try{
            ...
            if(storedRole == null || storedRole.isBlank()){
                storedRole = "JOB_SEEKER";
                user.setRole(storedRole);
                usersTableRepository.save(user);
            }
            if(!storedRole.equals(req.getRole())){
                return new ResponseEntity<>("Selected role doesn't match account",HttpStatus.UNAUTHORIZED);
            }
            ...
            loginResponse loginRes=new loginResponse(user.getUsername(), user.getPreviousResults(), storedRole);
